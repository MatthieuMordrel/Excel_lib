from sympy import symbols, expand, factor, simplify, solve, Symbol, Expr
import sympy as sp
...
def test_sympy_expressions() -> None:
    ...
    x, y, z = symbols('x y z')  # type: Tuple[Symbol, Symbol, Symbol]
    ...
    expanded: Expr = expand(expr2)
    print(f"Expanded form: {expanded}")
    
    # Simplify the expression
    simplified: Expr = simplify(expanded)
    ...
    # Substitute values
    result1: sp.Number = expr3.subs([(x, 2), (y, 3), (z, 4)])
    print(f"After substitution (x=2, y=3, z=4): {result1}")
    
    # Substitute expressions
    result2: Expr = expr3.subs(x, y + 1)
